<template>
    <div>
        <div class="form-group">
            <label fo="email">Your email</label>
            <span class="error">{{Error}}</span>
            <input type="email"  class="form-control" disabled @keyup.enter="resetPassword()" v-model="email" id="email" >
        </div>

        <span style="display: none" data-dismiss="modal"></span>

        <button class="btn" id="submitBtn" @click="resetPassword()"  style="float: left">Unsubscribe</button>
        ...
        methods:{
            resetPassword(){
                if(this.email !== "" ){
                    ...
                    this.Error=""
                    _('email').style.border = "1px solid #954b4b";
                    $.get('/api/unsubscribe/'+this.email, (response)=>{
                        if(response !== 'Email not exists'){
                            _('submitBtn').innerHTML =  "Unsubscribe"
                            this.titleAndId = ['Successfully',response.name,response.surname,response.image,response.email]
                            if(this.$refs.desktop.clientWidth > 1) {
                                buss.$emit('DesktopUnsubscribe', this.titleAndId );
                                _('submitBtn').innerHTML =  "Unsubscribe"
                            }else{
                                buss.$emit('MobileUnsubscribe',this.titleAndId);
                                _('submitBtn').innerHTML =  "Unsubscribe"
                            }
